  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-[#1a2e2e]" style={{ fontFamily: 'DM Serif Display, serif' }}>Donors</h1>
      ...
      <div className="bg-white rounded-2xl border border-[#e2ece8] overflow-hidden" style={{ boxShadow: '0 2px 12px rgba(15,92,92,0.06)' }}>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#F5F0E8]">
                {['Donor','Status','Phone','Joined','Actions'].map(h => (
        ...
        {!loading && filtered.length === 0 && <div className="text-center py-10 text-sm text-[#6b8a82]">No donors found.</div>}
      </div>

      <AddUserModal open={modalOpen} onClose={() => setModalOpen(false)} onAdd={handleAdd} fixedRole="Donor" />
