// pages/Donors.jsx — Donors only, Add User forces role=Donor
...
const INITIAL = [
  { id:1, name:'Ahmed Benali', email:'ahmed@gmail.com', phone:'+213 550 111', status:'active',  joined:'2024-01-12', donations:14, avatar:'AB' },
  { id:2, name:'Nadia Saad',   email:'nadia@gmail.com', phone:'+213 660 222', status:'active',  joined:'2024-04-22', donations:22, avatar:'NS' },
  { id:3, name:'Bilal Hamid',  email:'bilal@gmail.com', phone:'+213 662 333', status:'pending', joined:'2024-06-15', donations:8,  avatar:'BH' },
];
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-[#1a2e2e]"
            style={{ fontFamily: 'DM Serif Display, serif' }}>Donors</h1>
          <p className="text-sm text-[#6b8a82] mt-0.5">{donors.length} registered food donors</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-[#e2ece8] overflow-hidden"
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-[#F5F0E8]">
              {['Donor','Status','Phone','Joined','Donations','Action'].map(h => (
              ...
              <tr key={u.id} className="border-t border-[#e2ece8] hover:bg-[#FAF9F7]">
                <td className="px-5 py-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#0F5C5C] flex items-center justify-center text-white text-xs font-semibold flex-shrink-0">{u.avatar}</div>
                    <div>
                ...
                <td className="px-5 py-3 text-sm font-semibold text-[#0F5C5C]">{u.donations}</td>
                <td className="px-5 py-3">
        ...
        {filtered.length === 0 && <div className="text-center py-10 text-sm text-[#6b8a82]">No donors found.</div>}
      </div>

      <AddUserModal
        open={modalOpen}
        onClose={() => setModalOpen(false)}
        onAdd={user => setDonors(p => [...p, { ...user, donations: 0 }])}
        fixedRole="Donor"
      />
      ...
      {confirm && (
        <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-50" onClick={() => setConfirm(null)}>
          <div className="bg-white rounded-2xl p-7 w-72 shadow-2xl text-center" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="font-semibold text-[#1a2e2e] mb-2">Delete donor?</h3>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => { setDonors(p => p.filter(u => u.id !== confirm)); setConfirm(null); }} className="flex-1 py-2 rounded-xl bg-[#C96E4A] text-white text-sm font-semibold cursor-pointer border-none">Delete</button>
            </div>
          </div>
        </div>
      )}
