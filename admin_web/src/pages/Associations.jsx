// pages/Associations.jsx — filters users with role ASSOCIATION from /api/v1/admin/users
...
export default function Associations() {
  ...
  async function fetchAssociations() {
    ...
    try {
      ...
      setAssociations(all.filter(u => (u.role ?? u.roles?.[0] ?? '').toUpperCase() === 'COLLECTIVITE'));
  ...
  async function handleAdd(userData) {
    try {
      await adminUsers.create({ ...userData, role: 'COLLECTIVITE' });
  ...
  async function handleDelete(id) {
    try {
      ...
      setAssociations(p => p.filter(u => (u._id ?? u.id) !== id));
    } catch (e) { alert('Failed: ' + e.message); }
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={fetchAssociations} className="px-3 py-2.5 rounded-xl text-sm border border-[#e2ece8] bg-white text-[#6b8a82] cursor-pointer">↻</button>
